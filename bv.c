...
BitVector *bv_create(uint32_t length) {
    ...
    if (v) {
        ...
        uint32_t min_elems = length / BYTE + 1; // to get size for array (credits: based on lab doc)
...
void bv_set_bit(BitVector *v, uint32_t i) {
    if (i < bv_length(v)) // cant set if i > length vector
        v->vector[i / BYTE]
            |= ((uint8_t) 1 << (i % BYTE)); // based on hints from set.c given in asgn3 and curr lab
    return;
}
...
uint8_t bv_get_bit(BitVector *v, uint32_t i) {
    return (v->vector[i / BYTE] & ((uint8_t) 1 << (i % BYTE)))
               ? 1
               : 0; // based on hints from set.c given in asgn3 and curr lab
}

/* XORs the bit at index i with bit */
void bv_xor_bit(BitVector *v, uint32_t i, uint8_t bit) {
    v->vector[i / BYTE] ^= ((uint8_t) bit << (i % BYTE));
    return;
}
